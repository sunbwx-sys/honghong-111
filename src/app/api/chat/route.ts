...
import { assertDeepSeekEnv, safeLogError, sanitizeSecrets } from '@/lib/utils';
...
async function callDeepSeek(
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `DeepSeek API error ${response.status}: ${sanitizeSecrets(errText)}`,
    );
  }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!assertDeepSeekEnv('POST /api/chat')) {
      const fallback = getFallbackResponse(affection, step, isGameOver, won);
    ...
    const chatHistory: DeepSeekMessage[] = messages.map((msg) => ({
...
长度：20-40字。`;
      ...
      const llmMessages: DeepSeekMessage[] = [
        { role: 'system', content: systemPrompt },
        { role: 'user', content: '请回复：' },
      ];

      const content = await callDeepSeek(llmMessages, 0.9);

      return NextResponse.json({
        partnerMessage: content.trim(),
        options: [],
      });
    }
...
- 奇葩选项要真的很好笑，让人想分享给朋友`;
    ...
    const messagesForLLM: DeepSeekMessage[] = [
      { role: 'system', content: systemPrompt },
      ...chatHistory,
    ];
    ...
    const content = await callDeepSeek(messagesForLLM, 1.0);
    ...
    try {
      ...
      console.error('[POST /api/chat] Raw response preview:', sanitizeSecrets(content));
