...
app.prepare().then(async () => {
  ...
  try {
    console.log('---[ 启动自检：DeepSeek API 环境变量 ] ---');
    const { assertDeepSeekEnv } = await import('@/lib/utils');
    const ok = assertDeepSeekEnv('server-prepare');
    console.log(
      ok
        ? '  ✅ DEEPSEEK_API_KEY 已配置。'
        : '  ⚠️ 缺少 DEEPSEEK_API_KEY，AI 对话功能会走默认回复降级。',
    );
  } catch (err) {
    safeLogError('server prepare DeepSeek', err);
  }
