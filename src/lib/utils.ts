...
/**
 * 检查 DeepSeek API 所需的环境变量是否配置。
 *
 * 只需要 DEEPSEEK_API_KEY 一个变量。
 * 若未配置，AI 对话功能会走默认回复的降级逻辑。
 */
export function assertDeepSeekEnv(callerName: string): boolean {
  const required: Array<keyof NodeJS.ProcessEnv> = ['DEEPSEEK_API_KEY'];

  const missing = required.filter((k) => !process.env[k]);
  if (missing.length === 0) return true;

  console.error(
    `[${callerName}] ⚠️ 缺少 DeepSeek API 必需的环境变量：${missing.join(', ')}。` +
      '请在 .env.local 或部署平台的「环境变量」设置中添加 DEEPSEEK_API_KEY，' +
      '否则 AI 对话功能会持续走默认回复的降级逻辑。',
  );
  return false;
}
